...
class SigmaDSP
{
  public:
    ...
    SigmaDSP(I2C_HandleTypeDef *hi2c, uint8_t i2cAddress, uint8_t device, GPIO_TypeDef* resetGPIO, uint16_t resetPin, UART_HandleTypeDef *uart, void (*addMessage)(char* msg));
...
//    void i2cClock(uint32_t clock);
    ...
    void mute(uint16_t startMemoryAddress,          bool mute, uint8_t channels = 1);
    void twoChannelGain(uint16_t startMemoryAddress, int gain1, int gain2);
