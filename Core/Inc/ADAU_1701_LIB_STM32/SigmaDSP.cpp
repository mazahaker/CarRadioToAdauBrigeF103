...
void SigmaDSP::twoChannelGain(uint16_t startMemoryAddress, int gain1, int gain2) {
	...
	//	float _gain1 = pow(10, gain1 / 20); // 10^(dB / 20)
...
void SigmaDSP::squareSource(uint16_t startMemoryAddress, float frequency) {
	sineSource(startMemoryAddress, frequency);
}
void SigmaDSP::squareSource(uint16_t startMemoryAddress, int32_t frequency) {
	sineSource(startMemoryAddress, frequency);
}
...
#else
void SigmaDSP::safeload_writeRegister(uint16_t memoryAddress, int data, bool finished) {
	safeload_writeRegister(memoryAddress, (int32_t) data, finished);
}
#endif
void SigmaDSP::safeload_writeRegister(uint16_t memoryAddress, uint32_t data, bool finished) {
	safeload_writeRegister(memoryAddress, (int32_t) data, finished);
}
void SigmaDSP::safeload_writeRegister(uint16_t memoryAddress, uint16_t data, bool finished) {
	safeload_writeRegister(memoryAddress, (int32_t) data, finished);
}
void SigmaDSP::safeload_writeRegister(uint16_t memoryAddress, uint8_t data, bool finished) {
	safeload_writeRegister(memoryAddress, (int32_t) data, finished);
...
/***************************************
 ...
 Returns:  None
 ***************************************/
void SigmaDSP::writeRegisterBlock(uint16_t address, uint16_t length, const uint8_t *data, uint8_t registerSize) {
	//		uint8_t MSByte = 0;
	//		uint8_t LSByte = 0;
	...
	// Run until all bytes are sent
	while (bytesSent < length) {
		uint8_t packageOfData[registerSize];
		for (uint8_t i = 0; i < registerSize; i++) // Send n bytes
				{
			packageOfData[i] = data[bytesSent];
			bytesSent++;
		}
		writeRegister(address, registerSize, packageOfData);
 ...
 ***************************************/
int32_t SigmaDSP::readBack(uint16_t address, uint16_t readout, uint8_t numberOfBytes) {
	//	uint8_t LSByte = (uint8_t)memoryAddress & 0xFF;
	//	uint8_t MSByte = memoryAddress >> 8;
	//
	//	_WireObject.beginTransmission(_dspAddress); // Begin write
	...
	uint8_t package[2];
	package[0] = readout >> 8;
	package[1] = (uint8_t) readout & 0xFF;

	writeRegister(address, sizeof(package), package);

	uint8_t emptyData[] = { };
	writeRegister(address, sizeof(emptyData), emptyData);

	uint8_t buff[numberOfBytes];
	for (int i = 0; i < numberOfBytes; i++) {
		buff[i] = -1;
	}

	if (HAL_OK != HAL_I2C_Master_Receive(_hi2c, _dspAddress, buff, numberOfBytes, 10)) {
		return -1;
	}
...
/***************************************
 ...
 Inputs:   dspRegister registerAddres;       Register address
 uint8_t numberOfBytes; Number of bytes to read from the register
 Returns:  uint16_t value;        Value of the register
 ***************************************/
uint32_t SigmaDSP::readRegister(dspRegister address, uint8_t numberOfBytes) {
